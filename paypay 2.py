...
class PaypayCog(commands.Cog):
    ...
    async def _require_ops_token_entry(self, user_id: int | str):
        entry = get_paypay_entry(user_id) or {}
    ...
    def _result_message(self, result: dict, default: str) -> str:
        return result.get("header", {}).get("resultMessage") or default
    ...
    def _clear_saved_tokens(self, user_id: int | str):
        entry = get_paypay_entry(user_id)
        if not entry:
            return
        changed = False
    ...
    def _clear_saved_ops_tokens(self, user_id: int | str):
        entry = get_paypay_entry(user_id)
        if not entry:
            return
        changed = False
    ...
    async def _run_token_api(self, user_id: int | str, api_call, default_error: str):
        entry = await self._require_token_entry(user_id)
        result = await api_call(entry["access_token"], self._entry_uuid(entry))
        if self._is_revoked_result(result):
            self._clear_saved_tokens(user_id)
    ...
    async def _run_ops_token_api(self, user_id: int | str, api_call, default_error: str):
        entry = await self._require_ops_token_entry(user_id)
        ...
        if result_code in {"S0001", "S1003"} or self._is_revoked_result(result):
            ...
            if not refresh_token:
                self._clear_saved_ops_tokens(user_id)
    ...
    async def paypay_register(
        ...
        if result.get("access_token"):
            current = get_paypay_entry(interaction.user.id) or {}
            current.update({
                "phone": phone,
                "password": password,
            ...
            upsert_paypay_entry(interaction.user.id, current)
            embed = discord.Embed(
                title="PayPay登録完了",
                description="PayPayアカウント情報の登録が完了しました。",
                color=discord.Color.green()
    ...
    async def paypay_defined(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        try:
            _, result = await self._run_token_api(
                interaction.user.id,
                paypayu.get_balance,
                "利用可能額を取得できませんでした。"
            )
            ...
            await interaction.followup.send("❌ 利用可能額を取得できませんでした。", ephemeral=True)
    ...
    async def paypay_claimlink(self, interaction: discord.Interaction):
        ...
        try:
            _, result = await self._run_token_api(
                interaction.user.id,
                paypayu.create_mycode,
                "請求リンクの取得に失敗しました。"
            )
            payload = result.get("payload", {})
            link = payload.get("link") or payload.get("linkUrl") or payload.get("deeplink") or payload.get("codeUrl") or payload.get("p2pCode")
    ...
    async def paypay_profile(self, interaction: discord.Interaction):
        ...
        try:
            ...
            payload = result.get("payload", {})
            profile = payload.get("userProfile", payload)
            ...
            for label, key in (
                ...
                if profile.get(key):
                    embed.add_field(name=label, value=f"```{profile[key]}```", inline=False)
            await interaction.followup.send(embed=embed, ephemeral=True)
        except ValueError as e:
            await interaction.followup.send(f"❌ {e}", ephemeral=True)
        except Exception:
    ...
    async def paypay_history(self, interaction: discord.Interaction, limit: app_commands.Range[int, 1, 20] = 10):
        ...
        try:
            _, result = await self._run_token_api(
                interaction.user.id,
                lambda access_token, uuid_value: paypayu.get_history(access_token, uuid_value, limit),
                "履歴取得に失敗しました。"
            )
            payload = result.get("payload", {})
            items = payload.get("histories") or payload.get("history") or payload.get("transactions") or payload.get("paymentHistory") or []
            ...
            await interaction.followup.send("❌ 履歴を取得できませんでした。", ephemeral=True)
    ...
    async def paypay_createlink(self, interaction: discord.Interaction, amount: app_commands.Range[int, 1, 500000], passcode: str | None = None):
        ...
        try:
            _, result = await self._run_token_api(
                interaction.user.id,
                lambda access_token, uuid_value: paypayu.create_link(access_token, uuid_value, amount, passcode),
                "送金リンクの作成に失敗しました。"
            )
            payload = result.get("payload", {})
            ...
            await interaction.followup.send("❌ 送金リンクを作成できませんでした。", ephemeral=True)
